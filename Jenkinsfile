...
pipeline {
    ...
    stages {
        ...
        stage('Staging') {
            ...
            steps {
                ...
                withCredentials([
                    ...
                    script {
                        def statusCode = sh(script: "bash ./print_env.sh > ${WORKSPACE}/.env", returnStatus: true)
                        echo "statusCode: ${statusCode}"
                    }
        ...
        stage('Health check') {
            ...
            steps {
                ...
                docker_ps_health_check(['telemetry_nginx','telemetry_promtail'])
            }
        }
    ...
    post {
        success {
            archiveArtifacts "docker-compose.yml, start.sh, stop.sh, env.example"
        }
        always {
            echo 'cleaning'
